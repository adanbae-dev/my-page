...
사이트가 방문자에 대해 수집하는 것도 공개 정책의 일부다. 원본에 이 항목이
없었고, 없으면 나중에 조용히 늘어난다.

| 수집한다 | 수집하지 않는다 |
|---|---|
| 어떤 경로가 얼마나 읽혔는지 | 쿠키 — 하나도 쓰지 않는다 |
| 하루에 몇 번 열렸는지 | 개인을 식별하는 값 |
| 나라 · 기기 · 브라우저 (집계) | 로그인 · 계정 — 존재하지 않는다 |
| 어디서 들어왔는지 (referer) | 방문자 간 추적, 광고 프로파일 |

Cloudflare Web Analytics 로 받는다. 쿠키를 쓰지 않고, 이 사이트에는 로그인도
계정도 없으므로 **한 사람을 두 방문에 걸쳐 이을 수단 자체가 없다.**

`Permissions-Policy: interest-cohort=()` 는 그 전부터 있었다. 브라우저가
관심사 그룹을 계산해 광고에 쓰는 것을 거부하는 헤더다. 수집을 시작하면서
그 헤더를 유지하는 것이 앞뒤가 맞는지 확인했고, 맞는다 — 페이지가 얼마나
읽혔는지 세는 것과 사람을 프로파일링하는 것은 다른 일이다.

**대가는 CSP 에 뚫은 구멍 하나다.** 비컨이 엣지에서 주입되므로
`script-src 'self'` 가 거절한다. 그 하나를 열되, `pnpm check:release` 가
**외부 origin 이 정확히 그 하나인지** 검사한다. 정책이 약해지는 것은 문서화된
첫 예외 때문이 아니라 아무도 눈치채지 못한 두 번째 예외 때문이다.

자세한 것은 `docs/PRODUCTION.md` 의 Observability 절에 있다.
