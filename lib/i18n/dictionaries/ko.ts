...
export const ko = {
  ...
  seo: {
    ...
    direct:
      '중개사무소 밀도가 직거래율을 설명하는 것처럼 보이지만, 인구가 더 잘 설명합니다. {plotted}개 시군구를 두 가지로 설명한 로그-로그 산점도 두 장.',
    renewal:
      '갱신요구권을 쓴 전세 갱신과 안 쓴 갱신의 보증금 인상률 분포. 중앙값은 {medianUp}% 대 {medianDown}%로 거의 같고, 5%를 넘는 비율은 {overCapUp}% 대 {overCapDown}%입니다.',
    prices:
    ...
    reach:
      '아파트 매매 중개는 {national}%가 매물과 같은 시군구에서 일어납니다. 흐름 지도를 그리려다 그리지 않은 이유와, 그 대신 남은 {measured}곳의 분포·거리·예외.',
  },
  ...
  direct: {
    ...
    note:
      '한 장이 아니라 두 장인 이유는, 이 페이지의 발견이 상관이 아니라 상관의 비교이기 때문입니다. 왼쪽만 실으면 참이면서 오해를 부릅니다. 같은 y축에 같은 점을 놓으면, 왼쪽 이야기가 오른쪽에도 그대로 살아 있는 게 보입니다 — 그게 "부분적으로 대리변수"의 모습입니다. 두 축 모두 로그입니다. 직거래율이 {minRate}%에서 {maxRate}%까지, 인구도 자릿수를 넘나들어서 선형 축에서는 한 곳이 구석에 홀로 앉고 나머지 {others}개가 얼룩이 됩니다. 그러면 여기 적힌 상관계수가 나라가 아니라 그 한 곳에 대한 진술이 됩니다. 추세선은 긋지 않았습니다. 선은 원인에 대한 주장처럼 보이는데, 이 페이지의 요지는 어느 쪽도 원인을 세우지 못한다는 것입니다.',
    caption: '{plotted}개 시군구 · 아파트 매매 {deals}건 · {from} ~ {to} 신고분',
    summary:
      '{plotted}개 시군구의 직거래 비율을 두 가지로 설명한 로그-로그 산점도. 왼쪽은 인구 1만 명당 중개사무소 수, 오른쪽은 인구입니다. 오른쪽의 상관이 더 강합니다.',
    source:
  ...
  prices: {
    ...
    unmeasuredWhy:
      '창의 양 끝에서 같은 단지가 거래되지 않은 시군구입니다. {thin}곳은 짝이 있지만 {floor}개에 못 미쳤고, {nopairs}곳은 짝이 아예 없었습니다 — 반복되지 않는 단지에서만 거래됐다는 뜻이고, 신고가 없었다는 뜻이 아닙니다. 폐지된 지역코드도 정상 응답에 0건을 답해서 죽은 코드와 조용한 시장이 화면에서 같아 보이므로, 창이 통째로 빈 시군구가 있으면 빌드가 실패하게 해 뒀습니다. 구멍을 뚫지 않고 중립색 칸으로 남겼습니다 — 이 지도가 읽히는 이유는 나라 모양이고, 증거가 없다는 뜻으로 칸을 비우면 "변동이 없다"로 읽힙니다. 둘은 다른 말입니다.',
  ...
  reach: {
    heading: '중개 반경',
    lead:
      '아파트 매매 중개 {legs}건 가운데 {national}%가 매물과 같은 시군구의 사무소에서 나왔습니다. 시군구별 중앙값은 {median}%. 경계를 넘은 {externalShare}%도 중앙값 {distMedian}km, {within20}%가 20km 안입니다 — 흐름이 아니라 옆 동네입니다.',

    dotsHeading: '시군구마다 점 하나',
    note:
      '히스토그램이 아닙니다. 값이 {min}%에서 {max}%까지인데 {measured}곳의 5분위가 벌써 {p5}% — 스무 곳 중 열아홉이 그 위입니다. 덩어리가 보일 만큼 넓은 구간을 잡으면 그 아래가 전부 한 칸에 들어갑니다. 그런데 이 페이지의 유일한 진짜 예외가 바로 그 칸에 삽니다. 점 하나씩 찍으면 숨을 곳이 없습니다. 축도 0에서 100까지 그대로 두었습니다 — 데이터가 있는 곳으로 잘라내면 없는 산포가 있어 보입니다. 비어 있는 왼쪽 절반이 이 페이지의 발견입니다.',
    caption: '자기지역 중개 비율, 시군구 {measured}곳 · 다리 {floor}개 이상',
    summary:
      '자기지역 중개 비율을 0에서 100까지의 축에 시군구 하나당 점 하나로 찍은 그림. {measured}곳이 대부분 축 오른쪽 끝에 몰려 있고 중앙값은 {median}%, 5분위는 {p5}%, 95분위는 {p95}%입니다. 가장 낮은 곳은 {min}%로 혼자 왼쪽에 떨어져 있습니다.',
    source: '국토교통부 아파트 매매 실거래가 · 중개사무소 소재지 · {from}~{to}',
    medianLabel: '중앙',
    dot: '{rate}%',

    distanceHeading: '넘어가도 멀리 가지 않는다',
    distanceWhy:
      '경계를 넘은 다리 {external}건의 거리입니다. 중앙값 {distMedian}km, 90분위 {distP90}km, 최대 {distMax}km. 거리는 시군구 중심점 사이 대권거리이고, 중심점은 읍면동 중심점을 시군구별로 평균한 값입니다.',
    bandsCaption: '외부 다리 {external}건의 거리 분포, 누적',
    bandTerm: '거리',
    bandShare: '누적 비율',
    bandRow: '{to}km 이내',

    lowHeading: '예외는 인접이고, 예외의 예외는 하나다',
    lowWhy:
      '자기지역 비율이 가장 낮은 {measured}곳 중 열두 곳입니다. 최대 유입 열을 보면 대개 붙어 있는 시군구입니다 — 같은 시의 다른 구, 또는 경계를 맞댄 옆 시. 하나만 다릅니다: {sgg}는 {rate}%인데 들어온 중개의 {originShare}%가 {originKm}km 떨어진 {origin}에서 왔습니다. 거리로 설명되지 않는 유일한 칸이고, 이 표는 그 한 칸을 보이게 하려고 있습니다.',
    lowCaption: '자기지역 비율이 낮은 열두 곳과, 각 곳에 가장 많이 들어온 중개사무소 소재지',
    colDistrict: '시군구',
    colRate: '자기지역',
    colLegs: '다리',
    colOrigin: '최대 유입',
    ...
    limitsHead: '이 페이지가 하지 않은 것',
    arcTerm: '흐름 지도를 안 그렸다',
    arcWhy:
      '매매 신고마다 중개사무소 소재지가 있으니 원산지-목적지 행렬이 그대로 나오고, 떠오르는 그림은 전국을 잇는 호(arc)입니다. 재고 나서 그리지 않았습니다: 서로 다른 외부 짝이 {flows}개인데 가장 굵은 열 개가 외부 물량의 {topTen}%밖에 안 됩니다. 나머지는 얇게 흩어져 있습니다. 선 오천 개를 그려 전체의 {externalShare}%를 보여주면, 독자는 중개가 전국을 돌아다니는 것을 봤다고 생각하며 페이지를 떠납니다. 답할 수 없는 질문에 답하는 예쁜 그림은 그림이 없는 것보다 나쁩니다.',
    coTerm: '공동중개율이 아니다',
    coWhy:
      '소재지 필드에 시군구가 둘 적힌 신고가 {coPairs}건, 전체의 {coShare}%입니다. 그 가운데 서로 다른 시군구가 {coCross}건, 같은 시군구를 두 번 적은 것이 {coSame}건입니다. 사무소를 기록하는 필드라면 같은 구 짝이 많이 나와야 합니다 — 한 동네 사무소 둘이 함께 중개하는 건 흔한 일입니다. 그러니 이 필드가 기록하는 것은 사무소가 아니라 걸린 시군구의 집합이고, {coShare}%는 공동중개율이 아니라 공동중개가 경계를 넘은 비율입니다. 같은 시군구 안의 공동중개는 이 자료로 셀 수 없습니다. 비율이 아니라 개수로 적은 이유가 있습니다: {coSame}/{coPairs}은 소수점 한 자리에서 100.0%로 반올림되고, 그 100을 "하나도 없다"로 읽은 적이 있습니다. 개수는 반올림되지 않습니다.',
    legTerm: '거래가 아니라 다리',
    legWhy:
      '단위는 신고 {records}건이 아니라 다리 {legs}개입니다. 소재지가 쉼표로 나뉘면 항목 하나가 다리 하나입니다. 거래로 세면 공동중개 한 건이 먼저 적힌 사무소에만 잡혀서, 경계를 넘은 중개가 실제보다 적게 보입니다.',
    floorTerm: '{belowFloor}곳은 안 그렸다',
    floorWhy:
      '다리가 {floor}개 미만인 시군구 {belowFloor}곳과, 중개거래가 아예 없는 {noLegs}곳은 점을 찍지 않았습니다. 마흔 개에 기댄 비율과 삼천 개에 기댄 비율이 화면에서 같은 점이라, 표의 다리 열을 함께 실었습니다.',
    centroidTerm: '중심점은 근사값',
    centroidWhy:
      '시군구 중심점은 그 안의 읍면동 중심점을 단순 평균한 값입니다. 면적 가중이 아니어서, 도심에 동이 촘촘하고 주변에 면이 크게 붙은 시는 중심이 도심 쪽으로 당겨집니다. 사무소가 실제로 있는 쪽이라 이 페이지의 용도에는 맞지만, 표의 km는 사무소 사이 거리가 아니라 시군구 사이 거리입니다.',
