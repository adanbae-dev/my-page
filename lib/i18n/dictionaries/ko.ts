...
export const ko = {
  ...
  districts: {
    ...
    note:
      '육각형 크기가 값입니다. 시도 지도는 아래에서 차오르는 막대였는데 여기는 크기입니다 — 지킬 라벨이 없고 이 크기의 막대는 얼룩이 됩니다. 크기로 인코딩하면 도형이 양방향으로 자라므로, 면적이 값에 비례하려면 반지름이 제곱근을 따라야 합니다. 시도 지도의 선형과 여기의 제곱근은 어긋난 게 아니라 다른 성질로 인코딩하는 것입니다.',
    ...
    placedTerm: '자리가 밀렸다',
    placedWhy:
      '평균 2.01칸, 약 47km 밀렸고 가장 심한 곳은 6.02칸, 138km 입니다. 가장 많이 밀린 여덟 곳 중 일곱이 하남·강동·남양주·송파·구리·광주·중랑 — 서울 동쪽, 구가 가장 촘촘한 곳입니다. 이건 맞추기에 실패한 게 아니라 카토그램의 정의입니다. 한 칸에 시군구 하나씩 주면 모든 시군구가 같은 면적을 갖게 되므로, 수도권은 국토에서 차지하는 것보다 그림에서 더 넓은 자리를 차지해야 하고 누군가는 비켜야 합니다. 이전 판은 바다든 어디든 가장 가까운 빈 칸을 쓰게 해서 밀림이 절반(1.05칸, 23km)이었지만, 나라 모양이 아니라 점 무더기로 보였습니다.',
    tiledTerm: '국토를 칸으로 나눴다',
    tiledWhy:
      '경계 폴리곤(시군구 254개, 외곽선 4,506개, 점 120만 개)을 전부 읽어, 육각 격자에서 중심이 육지에 떨어지는 칸을 골랐습니다 — 236칸. 여기에 섬이 육각형 하나보다 작은 시군구 10곳의 자리를 더해 246칸이고, 245개가 들어가 한 칸이 빕니다. 어느 칸에 누가 앉을지는 헝가리안 알고리즘으로 제곱거리 합을 최소화해 정했습니다. 손으로 놓은 자리는 하나도 없고, 다시 돌리면 같은 파일이 나옵니다.',
    missingTerm: '11곳이 없다',
    missingWhy:
