...
export const ko = {
  ...
  districts: {
    heading: '시군구 카토그램',
    lead:
      '시도 열여섯 개를 245개로 쪼갠 지도입니다. 이름은 하나도 보이지 않습니다 — 육각형 반지름이 13px 라 글자가 들어가지 않습니다. 대신 분포가 보입니다.',
    mapHeading: '인구 1만 명당 중개사무소 · 시군구',
    note:
      '육각형 크기가 값입니다. 시도 지도는 아래에서 차오르는 막대였는데 여기는 크기입니다 — 지킬 라벨이 없고 13px 막대는 얼룩이 됩니다. 크기로 인코딩하면 도형이 양방향으로 자라므로, 면적이 값에 비례하려면 반지름이 제곱근을 따라야 합니다. 시도 지도의 선형과 여기의 제곱근은 어긋난 게 아니라 다른 성질로 인코딩하는 것입니다.',
    caption: '{n}개 시군구 · 육각형 크기가 인구 1만 명당 사무소 수입니다.',
    listing: '순위대로 전부 보기',
    summary:
      '대한민국 245개 시군구의 인구 1만 명당 중개사무소 수를 육각 격자로 그린 카토그램. 육각형이 클수록 값이 큽니다. 같은 숫자가 아래 목록에 순위대로 전부 있습니다.',
    source:
    ...
    placedTerm: '자리가 밀렸다',
    placedWhy:
      '30×35 격자 1,050칸에 245개를 놓았습니다. 중심점이 가장 가까운 빈 칸을 차지하는 방식이라 평균 1.05칸, 최대 4.60칸 밀립니다. 서울 25개 구와 청주 4개 구는 격자가 그들보다 성기기 때문에 전부 제자리에 앉을 수 없습니다. 격자를 키워도 38×44 에서 0.89칸으로 거의 나아지지 않아 여기서 멈췄습니다.',
    missingTerm: '11곳이 없다',
    missingWhy:
    ...
    labelsTerm: '이름이 없다',
    labelsWhy:
      '245개에 이름을 쓸 수 없습니다. 이 지도는 "세종의 비율이 얼마인가" 가 아니라 "어디가 높은가" 에 답합니다. 이름과 숫자는 육각형 위에 포인터를 올리면 나오고, 아래 목록에 순위대로 전부 있습니다.',
