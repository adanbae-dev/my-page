...
export const ko = {
  ...
  renewal: {
    ...
    lead:
      '갱신요구권을 쓴 전세 갱신의 중앙 인상률은 4.9%, 안 쓴 쪽은 4.7%. 평균만 보면 상한이 아무 일도 안 한 것처럼 보입니다. 상한이 하는 일은 가운데가 아니라 천장에 있습니다.',
    chartHeading: '보증금 인상률 분포 · 갱신요구권 사용 여부',
    note:
      '두 분포를 하나의 축에 위아래로 겹쳤습니다. 옆에 나란히 놓으면 비슷하게 생긴 두 모양을 머릿속에서 겹쳐야 하는데, 한 축에 붙이면 한쪽은 벽이고 한쪽은 꼬리라는 게 눈에서 끝납니다. 두 계열의 계약 수가 1만 건 넘게 차이 나므로 각자 자기 계열 대비 비율로 그렸고, 픽셀 대 퍼센트 비율은 위아래가 같습니다 — 두 배 높은 막대는 두 배 흔합니다. 위아래에 각자 축척을 주면 그림은 고르게 차지만 47% 봉우리가 27% 봉우리처럼 보입니다.',
