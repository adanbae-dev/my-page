...
export const ko = {
  ...
  districts: {
    ...
    note:
      '색이 값입니다. 처음에는 육각형 크기였는데 읽히지 않았습니다 — 상위 네 곳(중구 39.8, 강남 37.0, 용산 34.0, 서초 31.3)이 최댓값을 끌고 가는 바람에, 나머지 80%가 반지름 0.36~0.72 사이에 전부 몰렸습니다. 196개가 서로 붙어 있는데 반지름이 두 배 차이면 같아 보입니다. 그래서 등량 7단계로 바꿨습니다 — 한 단계에 35개씩이라 값이 어떻게 뭉쳐 있든 그림이 범위를 다 씁니다. 대신 색에서 크기를 읽을 수는 없습니다("한 단계 위"가 "1만 명당 몇 곳 위"가 아닙니다). 그래서 경계값을 지도 아래에 적어 두고, 정확한 숫자는 칸을 짚으면 나옵니다.',
    caption: '{n}개 시군구 · 칸 하나가 시군구 하나이고, 색이 인구 1만 명당 사무소 수입니다.',
    legend: '등량 7단계 · 한 단계가 35개 시군구',
    listing: '순위대로 전부 보기',
    summary:
