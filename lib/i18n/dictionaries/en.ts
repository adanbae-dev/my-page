...
export const en = {
  ...
  districts: {
    ...
    note:
      'Colour is the value. It was hexagon size first, and it did not read: four districts in central Seoul (Jung-gu 39.8, Gangnam 37.0, Yongsan 34.0, Seocho 31.3) own the top of the range, which squeezed the other eighty percent of the country into radii between 0.36 and 0.72 of full. A two-fold difference in radius across 196 neighbouring tiles looks like no difference at all. Seven classes of equal count replaced it \u2014 35 districts per step, so the picture uses its whole range however the values bunch. What that costs is magnitude: a step up the ramp is "the next 35 districts", not "five more per ten thousand". So the breaks are printed under the map, and the exact number for any district is one hover away.',
    caption: '{n} districts · one cell per district, coloured by offices per 10,000 residents.',
    legend: 'Seven classes of equal count · 35 districts each',
